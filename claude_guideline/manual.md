...
- 모호한 수치를 단정 적용했다가 silent bug 가 발견되면 ([coding.md](coding.md) "상수 분리 원칙"), 수정 이력(v1/v2/v3 가설과 사유)을 해당 코드의 `code_updates/` 에 보존한다 ([coding.md](coding.md) §수정 이력 기록 — 코드 주석에는 이력을 남기지 않고 현재 채택값의 근거 1 줄만 남긴다).
