...
문서·코드에 경로를 기재할 때는 배포·이식 시 깨지지 않도록 다음을 지킨다:

- 절대 경로 금지 — 프로젝트 루트 기준 상대 경로로 기재한다.
- 프로젝트 밖 외부 경로는 `<placeholder>` 로 표기하고, 실제 값은 설치 문서·설정 파일에서 정의한다.
- 홈 디렉터리는 `$HOME` (또는 `~`) 로 표기하고 특정 사용자명을 하드코딩하지 않는다.
- 예외: 실측 로그·실행 출력 인용 등 사실 기록은 원문 그대로 유지한다 (본 규칙은 신규 작성 경로에 적용).
...
- **증분 작성**: 모듈 전체를 한 번에 채울 필요 없다 — 수정이 닿는 파일부터 파일 단위로 등재한다 (생성·갱신 의무는 coding.md). §커버 파일에 등재된 파일 목록을 유지해 미등재 파일을 식별한다.
- **확장 표 (옵션)**: 필요 시 같은 형식으로 상수 표(이름·값·의미·근거), ROS2 인터페이스 표(토픽/서비스/액션·타입·방향)를 추가할 수 있다.
- 형식:
...
<본 인벤토리에 등재 완료된 파일 목록 — 증분 작성 진행도>
