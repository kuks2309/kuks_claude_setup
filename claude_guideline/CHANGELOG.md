...
SIL/HIL 룰을 **실기 구동 규율**로 확장 — 중복 판정을 토픽 축으로 보강하고 기동 절차를 고정.

### 변경

- `claude_guideline/ros2.md`: **시험 전 노드 중복 실행 방지 (SIL/HIL)** → **실기·시험 기동 규율 (SIL/HIL)** 로 개편.
  - 적용 시점을 "시험(검정·성능 측정)" 에서 **측정 유무 무관 실기 구동 전 전부**로 확대.
  - 정리 대상 선별을 **2축**으로 — 노드 이름 겹침(ⓐ) 외에 **측정·제어 대상 토픽의 기존 발행자**(ⓑ, `ros2 topic info -v`)를 추가. remap 으로 노드 이름이 달라진 중복 발행자는 이름 대조로 잡히지 않는다.
  - **전량 종료는 채택하지 않음** — 드라이버 재초기화·안전 감시·진행 중 기록을 함께 파괴하고, 다중 호스트에서는 범위 정의가 불가능하다.
  - **고정 절차 기동** 신설 — 런치 파일을 하나로 합칠 필요는 없고(계층 런치는 ROS2 정상 구성), 절차서에 적힌 런치·순서 그대로만 올린다. 절차 밖 개별 `ros2 run` 얹기 금지.
  - **계층 얹기는 개발·디버그 한정** 신설 — 얹기 자체는 허용하되 그 상태의 수치·판정은 측정치로 보고 금지, 기록 시 "얹기 상태" 명시.
  - `ros2 node list` 가 daemon 캐시라는 한계를 명시하고 `ps aux` 병행을 요구.

### 트리거

1.16.0 이 사고에서 "측정 전 중복 검사" 만 도출하고 상위 대책(기동 구성 고정)을 남기지 않았다. 초안의 "전량 중지 + 단일 런치" 는 사용자 검토에서 기각 — 전량 중지는 실기에서 성립하지 않고(드라이버·안전·로깅 파괴), 단일 런치는 불필요한 재구조화다. 실제 실패 지점은 선별 여부가 아니라 **선별 기준이 노드 이름뿐이었던 것**이라는 판단에 따라 토픽 축을 추가했다.

### 호환성

minor bump. 기존 4단계는 5단계에 포섭(1↔ⓐ, 2, 4↔5 유지 / 3 인스턴스 1개 원칙 → 고정 절차 기동으로 대체). ROS2 프로젝트에만 적용.
...
- `claude_guideline/ros2.md`: **시험 전 노드 중복 실행 방지 (SIL/HIL)** 절 신설 — 기동 전 `ros2 node list`·`ps aux` 검사, 잔류 정리 후 daemon 재기동, 인스턴스 1개 원칙, 기동 후 `ros2 topic info -v` 발행자 수 확인의 4단계. ROS2 가 동일 이름 노드 중복 실행을 막지 않는다는 전제에서 기동 자체를 통제한다.

### 트리거

실사격(2026-08-02): `/mcl_pose` 발행자가 2개인 상태로 측정해 보고한 개선 수치(14.6 / 21.4 / 26.7 Hz)가 여러 인스턴스의 합계로 판명, 전량 무효 처리. 사후 측정 검증이 아니라 기동 전 중복 차단이 근본 대책이라는 사용자 판단에 따라 게이트 위치를 시험 시작 시점으로 확정.

### 호환성

minor bump. 신규 절 추가이며 기존 룰 변경 없음. ROS2 프로젝트에만 적용.
...
- `claude_guideline/coding.md` §코딩 스타일: **기본 중괄호 스타일 Allman** 신설 — formatter 설정·모듈 규약이 달리 정하지 않는 한 `if`/`for`/`while`/`switch`/함수 정의의 여는 중괄호 `{` 를 다음 줄에 단독 배치 (예시 코드 블록 포함). 기존 "Allman vs K&R 중 명시" 문구는 "별도 선언 없으면 Allman 기본" 으로 정리.

### 트리거

사용자 지시 (2026-08-01): "코딩 규칙에서 for( ) { } if( ) { } — 이런 스타일을 하도록 지시했는데 현재는 그렇지 않습니다" — Allman 지시가 규칙에 박제되지 않아 기본값 공백 상태였음을 확인, 삽입 문구 승인 후 명문화.

### 호환성

minor bump. 규칙 추가만 — 새로 작성하는 코드부터 적용. formatter 설정이 있는 프로젝트는 영향 없음 (§코딩 스타일 우선순위 불변).
