...
paragraph 내부의 레이아웃 캐시. 한글이 마지막으로 렌더링한 결과를 저장한다.
...
<hp:linesegarray>
  <hp:lineseg
    textStartSIDRef="0" vertPos="0" vertSize="1600"
    textHeight="1600" baseLineGap="1200" spacing="160"
    horzPos="0" horzSize="47688" tag="0"/>
</hp:linesegarray>
```
...
| 속성 | 단위 | 설명 |
|------|------|------|
| vertSize | 1/100pt | 줄 높이 |
| textHeight | 1/100pt | 텍스트 높이 |
| baseLineGap | 1/100pt | 베이스라인 간격 |
| spacing | % | 줄간격 백분율 |
| horzPos | HWPUNIT | 가로 시작 위치 |
| horzSize | HWPUNIT | 가로 크기 (줄 너비) |

> 텍스트 치환 후 반드시 제거. 제거해도 한글이 자동 재계산하므로 안전.
> 도구: `scripts/fix_linesegarray.py`
